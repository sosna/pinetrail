...
public enum Activity {
  ...
  HIKING(5.0) {
    /**
     * Determines the difficulty level out of the provided statistics about distance and elevation
     * differences.
     *
     * <p>The level the user considers to have will also be taken into account, as the trail
     * difficulty will be perceived differently by novices and experts.
     *
     * <p>For hiking, the formula was kindly provided by
    ...
    public Integer getDifficultyRating(
        ...
        final Statistics elevationStats,
        final Instant startPeriod,
        final Instant endPeriod) {
      final double rating =
          getWeight(userLevel) * elevationStats.getActiveUp().getSum() * METERS2FEET
              + distanceStats.getActive().getSum() * KM2MILES / (METERS2KM * getDivider(userLevel));
      final long days = ChronoUnit.DAYS.between(startPeriod, endPeriod) + 1; // At least one day
      final double ratingByDay = rating / days;
      return (int) ratingByDay;
    }
  ...
  Activity(final double averageSpeed) {
   ...
   *
   * <p>This allows to override the default expected average speed, based on previous activities.
   *
  ...
  /**
   * Determines the difficulty rating out of the provided statistics about distance and elevation
   * differences.
   *
   * <p>The level this user considers to have will also be taken into account, as the trail
   * difficulty will be perceived differently by novices and experts.
   *
   * <p>Currently this works only for hiking activities. For the other activities, 0 will be
   * returned,
   *
   * @param userLevel the level the user considers to have
   * @param distanceStats statistics about the trail distance
